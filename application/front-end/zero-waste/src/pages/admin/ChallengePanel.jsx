...
function ChallengePanel() {
  ...
  const {
    ...
    handlePreviousPage,
    deleteItem
  } = useAdminReports('challenge');

  const handleDelete = async (challenge_id) => {
    if (!window.confirm(t('admin.confirmDeleteChallenge', 'Are you sure you want to delete this challenge?'))) return;
    try {
      await deleteItem(challenge_id, "delete_media");
    } catch (e) {
      alert(t('admin.deleteChallengeFailed', "Failed to delete challenge."));
    }
  };
