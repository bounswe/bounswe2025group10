  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 space-y-4 sm:space-y-5">
      ...
      <section className="grid gap-3 sm:gap-4 grid-cols-1 lg:grid-cols-2">
        {/* Sustainability Tips */}
        <div
          className="p-3 sm:p-4 rounded-lg border h-full flex flex-col"
          style={{
            backgroundColor: currentTheme.background,
            borderColor: currentTheme.border
          }}
        >
          <h2
            className="text-base sm:text-lg font-bold mb-3 flex-none"
            style={{ color: currentTheme.text }}
          >
          ...
          <div className="flex flex-col gap-2 flex-1 overflow-y-auto min-h-0">
            {Array.isArray(sustainabilityTips) && sustainabilityTips.map((tip) => (
              <div
                key={tip.id}
                className="p-2.5 sm:p-3 rounded-lg border-l-2 border flex-grow flex-shrink-0"
                style={{
                  backgroundColor: currentTheme.hover,
                  borderColor: currentTheme.border,
              ...
              >
                <h3 className="font-bold text-lg mb-1" style={{ color: currentTheme.text }}>
                  {tip.title}
                </h3>
                <p className="text-xs opacity-80" style={{ color: currentTheme.text }}>
                  {tip.description}
                </p>
              </div>
            ))}
